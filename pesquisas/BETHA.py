...
class Betha():
	...
	def run(self):
		...
		for line in cidades:
			...
			try:
				...
				logger.info("BETHA (%s) finalizado. [%s]"%(self.cidade, self.documento))

			except TimeoutException:
				logger.warning("<Warning: BETHA - (%s) Tempo Limite excedido [%s]>"%(self.cidade, self.documento))
				pass
	...
	def search(self):
		...
		if self.tipo_documento == 'Jurídica':
